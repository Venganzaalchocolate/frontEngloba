...
export default function FormJobUp({ modal, charge, user, changeUser, closeModalEdit = () => { } }) {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: user?.phone || '',
    gender: user?.gender || '',
    work_schedule: Array.isArray(user?.work_schedule) ? (user.work_schedule[0] || '') : '',
          ...
          if (offerJob?.error) {
            ...
            const nameJob =
              (res?.jobs ?? [])
                ...
                .find((sc) => String(sc._id) === String(offerJob.jobId))?.name ?? '';
            offerJob.nameOffer = `${nameJob} - ${offerJob?.location?.toUpperCase() || ''}`;
            setOffer(offerJob);
          }
        }
      } finally {
  ...
  // ahora operamos sobre *_Id
  const addJob = useCallback(
    (v) => handleArrayChange('jobsId', [...new Set([...(formData.jobsId || []), v])]),
    [formData.jobsId, handleArrayChange]
  );
  const removeJob = useCallback(
    (v) => handleArrayChange('jobsId', (formData.jobsId || []).filter((x) => x !== v)),
    [formData.jobsId, handleArrayChange]
  );

  const addStudy = useCallback(
    (v) => handleArrayChange('studiesId', [...new Set([...(formData.studiesId || []), v])]),
    [formData.studiesId, handleArrayChange]
  );
  const removeStudy = useCallback(
    (v) => handleArrayChange('studiesId', (formData.studiesId || []).filter((x) => x !== v)),
    [formData.studiesId, handleArrayChange]
  );

  const addProv = useCallback(
    (v) => handleArrayChange('provincesId', [...new Set([...(formData.provincesId || []), v])]),
    [formData.provincesId, handleArrayChange]
  );
  const removeProv = useCallback(
    (v) => handleArrayChange('provincesId', (formData.provincesId || []).filter((x) => x !== v)),
    [formData.provincesId, handleArrayChange]
  );
  ...
  const [submitState, formAction, isSubmitting] = useActionState(
    async (_prevState, fd) => {
      // Recogemos como IDs y normalizamos
      ...
      const values = {
        ...
        disability:
          fd.get('hasDisability') === 'true' || fd.get('hasDisability') === 'on'
            ? fd.get('disability')
            : 0,
        offer: offerInfo?._id || '',
      };
      ...
      // El archivo lo mandamos aparte al helper
      delete values.file;
  ...
  return (
    <div className={styles.contenedor}>
      ...
      <form className={styles.formJob} action={formAction} onReset={handleReset}>
        ...
        {/* MultiSelects: ahora trabajando con IDs */}
        <MultiSelectGrouped
          label="Puestos de interés"
          enums={enums.jobs}
          ...
          disabled={isLoadingEnums}
        />
        {formErrors.jobsId && <small className={styles.errorMsg}>{formErrors.jobsId}</small>}
        ...
        <MultiSelectFlat
          ...
          enums={provincesFlat /* aplanado */}
          selected={formData.provincesId}
          onAdd={addProv}
          onRemove={removeProv}
          disabled={isLoadingEnums}
        />
        {formErrors.provincesId && <small className={styles.errorMsg}>{formErrors.provincesId}</small>}
        {formData.provincesId.map((v) => (
          <input key={`provincesId-${v}`} type="hidden" name="provincesId" value={v} />
        ))}
        ...
        <MultiSelectGrouped
          ...
          disabled={isLoadingEnums}
        />
        {formErrors.studiesId && <small className={styles.errorMsg}>{formErrors.studiesId}</small>}
        {formData.studiesId.map((v) => (
          <input key={`studiesId-${v}`} type="hidden" name="studiesId" value={v} />
        ))}

        {/* Sobre mí */}
        <div className={`${styles.field} ${styles.full}`}>
          <label htmlFor={`${idForm}-about`}>Sobre mí</label>
          <textarea id={`${idForm}-about`} name="about" value={formData.about} onChange={handleChange} />
        </div>
        ...
        <div className={`${styles.field} ${styles.full}`}>
          {formData.file ? (
            <div className={styles.fileInfo} aria-live="polite">
              ...
              <button type="button" className={styles.linkButton} onClick={removeSelectedFile}>
                Quitar
              </button>
            </div>
          ) : (
            <small className={styles.muted}>Ningún archivo seleccionado</small>
          )}

          <label>
            <input type="checkbox" name="terms" checked={formData.terms} onChange={handleChange} /> *Aceptar términos y
            condiciones:&nbsp;
            <Link to="https://engloba.org.es/privacidad"> política de privacidad </Link>
          </label>
          <input type="hidden" name="terms" value={boolToAttr(formData.terms)} />

          <small>
            *Para cumplir con la normativa vigente en protección de datos de carácter personal, tal y como se indica en
            la correspondiente cláusula informativa, tu C.V. se mantendrá en nuestra base de datos durante un periodo de
            dos años a contar desde la fecha de entrega, pasado este tiempo se borrarán tus datos.
          </small>
